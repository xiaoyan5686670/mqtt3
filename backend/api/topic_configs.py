...
def get_topic_configs(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """获取主题配置列表"""
...
def get_topic_config(config_id: int, db: Session = Depends(get_db)):
    """获取单个主题配置"""
...
def create_topic_config(config: TopicConfigCreate, db: Session = Depends(get_db)):
    """创建主题配置"""
    return topic_config_service_module.create_topic_config(db, config)


@router.put("/{config_id}", response_model=TopicConfig)
def update_topic_config(config_id: int, config_update: TopicConfigUpdate, db: Session = Depends(get_db)):
    """更新主题配置"""
...
def delete_topic_config(config_id: int, db: Session = Depends(get_db)):
    """删除主题配置"""
...
def activate_topic_config(config_id: int, db: Session = Depends(get_db)):
    """激活主题配置"""
    success = topic_config_service_module.activate_topic_config(db, config_id)
    if not success:
        raise HTTPException(status_code=404, detail="Topic Config not found")
