...
def migrate():
    ...
    try:
        ...
        print("Adding relay payload fields to topic_configs...")
        
        if 'relay_on_payload' not in columns:
            db.execute(text("ALTER TABLE topic_configs ADD COLUMN relay_on_payload TEXT"))
            print("Added relay_on_payload field")
        
        if 'relay_off_payload' not in columns:
            db.execute(text("ALTER TABLE topic_configs ADD COLUMN relay_off_payload TEXT"))
